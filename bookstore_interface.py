...
class Ui_MainWindow(object):
    ...
    def addPublisher(self):
        ...
        if '' in inputDict.values():
            self.OkMsgBox("warning", "fail", "Fields can't be empty!")

        elif publisher_exists:
            self.OkMsgBox("warning", "fail", " Publisher with this name found!")

        elif not inputDict['phone_number'].isdigit():
            self.OkMsgBox("warning", "fail", " Phone_Number is not accepted!")
 ...
 \
        ...
        if not '' in inputDict.values():
            ...
            if bookExists:
                ...
                try:
                    ...
                    self.OkMsgBox("warning", "fail", "something went wrong while uploading photo!")

        else:
            self.OkMsgBox("warning", "fail", "fields can not be empty!")

    # Opens a file dialog to select a book photo (jpg, gif, png, jpeg).Updates the UI with the selected photo's URL.
    def getPicture(self):
    ...
    # Remove displayed user objects from the UI
    ...
    # Update user information if changes detected
    def updateInfo(self):
    ...
    def setupUi(self, MainWindow):
        ...
        self.button_list_search = QtWidgets.QPushButton(self.frame_list_main)
        self.button_list_search.setGeometry(QtCore.QRect(450, 20, 80, 24))
        self.button_list_search.setObjectName("button_list_search")
        # search button event listener
        self.button_list_search.clicked.connect(lambda: self.search(self.input_list_search.text()))
        # filter combo boxes
        ...
        self.button_list_show = QtWidgets.QPushButton(self.frame_list_main)
        self.button_list_show.setGeometry(QtCore.QRect(10, 70, 75, 24))
        ...
        if self.is_admin:
            ...
            self.button_publisher_add = QtWidgets.QPushButton(self.formLayoutWidget)
            self.button_publisher_add.setObjectName("button_publisher_add")
            self.formLayout_publisher_main.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.button_publisher_add)
            # new publisher button event listener
            self.button_publisher_add.clicked.connect(lambda: self.addPublisher())
            ...
            self.button_addbook_submit = QtWidgets.QPushButton(self.add)
            self.button_addbook_submit.setObjectName("button_addbook_submit")
            self.gridLayout_addbook_main.addWidget(self.button_addbook_submit, 8, 1, 1, 1)
            # add book button event listener
            self.button_addbook_submit.clicked.connect(lambda: self.addBook())
            ...
            self.button_addbook_broswer = QtWidgets.QPushButton(self.frame_addbook_picture)
            self.button_addbook_broswer.setGeometry(QtCore.QRect(510, 10, 80, 21))
            ...
            self.button_user_search = QtWidgets.QPushButton(self.frame_user_main)
            self.button_user_search.setGeometry(QtCore.QRect(450, 20, 80, 24))
            self.button_user_search.setObjectName("button_user_search")
            # user search event handler
            self.button_user_search.clicked.connect(lambda: self.usersSearch(self.input_user_search.text()))
            ...
            self.button_user_show = QtWidgets.QPushButton(self.frame_user_main)
            self.button_user_show.setGeometry(QtCore.QRect(150, 20, 75, 24))
            self.button_user_show.setObjectName("button_user_show")
            # filter button creation
            userFilters = ['admin', 'newest', 'oldest', 'most_loyal', 'least_loyal']
            self.usersCombo = QtWidgets.QComboBox(self.frame_user_main)
            self.usersCombo.setGeometry(QtCore.QRect(20, 20, 100, 24))
            self.usersCombo.addItem("select.criteria")
            self.usersCombo.addItems(userFilters)
            # filter button event listener
            self.button_user_show.clicked.connect(lambda: self.userFilter())
        ...
        columns = ['Book_Added', 'User', 'Publisher', 'Purchase_Date', 'Quantity', 'Price', 'Total_Price']
        ...
        if self.is_admin:
            ...
            self.user_info_fram.setObjectName("user_info_frame")
            ...
            self.button_user_info_update = QtWidgets.QPushButton(self.user_info_fram)
            self.button_user_info_update.setGeometry(QtCore.QRect(230, 380, 80, 24))
            self.button_user_info_update.setObjectName("button_user_info_update")
            ...
            self.button_user_info_update.clicked.connect(lambda: self.updateInfo())

        self.button_logout = QtWidgets.QPushButton(self.centralwidget)
        self.button_logout.setGeometry(QtCore.QRect(800, 10, 80, 24))
        ...
        self.button_list_show.clicked.connect(MainWindow.update)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    # Establishes translation for the main window title
    def translateUi(self, MainWindow):
        ...
        if self.is_admin:
            ...
            self.label_publisher_name.setText(_translate("MainWindow", "Publisher_Name"))
            ...
            # self.input_user_info_lastname.setText(_translate("MainWindow", "SALMA"))
            # self.input_user_info_name.setText(_translate("MainWindow", "YASSER"))
            # self.input_user_info_number.setText(_translate("MainWindow", "123456"))
            # self.input_user_info_address.setPlainText(_translate("MainWindow", "UTMKL"))
            self.label_user_info_username.setText(_translate("MainWindow", "Username"))
            # self.input_user_info_username.setText(_translate("MainWindow", "SY")) #SALMA YASSER
